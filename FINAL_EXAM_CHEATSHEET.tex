...
\textbf{MEMORIZE:}
\begin{itemize}[leftmargin=*,itemsep=0pt]
\item Basic probability rules (complement, union)
\item Bayes' theorem structure
\item Normal standardization: $Z = (X-\mu)/\sigma$
\item CLT statement
\item Independence $\Rightarrow$ $\text{Cov} = 0$ (not vice versa)
\item For normal: independence $\Leftrightarrow$ $\rho = 0$
\end{itemize}

\textbf{LOOK UP:}
\begin{itemize}[leftmargin=*,itemsep=0pt]
\item Specific distribution formulas
\item MGF formulas
\item Jacobian computation details
\item Complex integration bounds
\item Normal table values
\end{itemize}

...
\subsection*{Emergency Quick Reference: Problem Phrase → Section}
{\small
\begin{itemize}[leftmargin=*,itemsep=0pt,topsep=0pt]
\item \textbf{``Gaussian''} → Normal! Sec 3.3
\item \textbf{``Gaussian vector''} → MVN! Sec 4.5
\item \textbf{``Independent components''} → $\rho=0$ for MVN, Sec 4.5
\item \textbf{``Large n'' / ``Approximate''} → CLT, Sec 6.1
\item \textbf{``i.i.d.''} → Independence, maybe CLT
\item \textbf{``Prior/Posterior''} → Bayesian, Sec 7.2
\item \textbf{``Update belief''} → Bayes' Theorem, Sec 1.3
\item \textbf{``Conjugate''} → Beta-Binomial, Sec 7.2
\item \textbf{``Stock price'' / ``$S_0 e^Z$''} → Lognormal, Sec 7.3
\item \textbf{``Mean $\theta$'' (Exp)} → $\lambda = 1/\theta$! Sec 3.4
\item \textbf{``Memoryless''} → Exponential, Sec 3.4
\item \textbf{``Arrival/Counting process''} → Poisson, Sec 2.3
\item \textbf{``Max/Min of n''} → Order Statistics, Sec 4.7
\item \textbf{``$\psi(t)$''} → MGF! Sec 5.1
\item \textbf{``Conditional distribution''} → Sec 4.2, 4.5
\item \textbf{``$E[X|Y]$''} → Conditional Expectation, Sec 7.1
\item \textbf{``Total winnings/games''} → CLT, Template B
\item \textbf{``Monty Hall''} → Bayesian, Sec 8.1
\item \textbf{``Defective rate''} → Bayesian, Sec 8.8
\end{itemize}
}
